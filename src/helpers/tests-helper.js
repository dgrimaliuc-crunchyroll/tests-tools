...
let methodRegexPattern =
  /[+\- ](\s*)?@[TP]\w+\s+(`?)(.+?)\2\s*\([\S\s]+?\n[+\- ]\1}/g;
let exceptTags = new Map(
  Object.entries(JSON.parse(process.env.TAGS_EXCEPTIONS ?? '{}'))
);
  ...
  return JSON.parse(readFile(allTestsJsonPath).replaceAll(/[\n|\t]+/g, '')).map(
    (t) => {
      ...
      if (process.env.VERIFY_CI === 'true') {
        if (t.runInCI) addTagRes.push('ci');
        else addTagRes.push('notCi');
      }
...
function getLocalTests() {
  ...
  let actualTags = [
    ...readFile(process.env.ACTUAL_TAGS_FILE).matchAll(/\w+.=.+/g),
  ]
    .map((it) => ['let ' + it.toString()])
    .join('\n');
...
function saveLocalTests(content) {
  writeIntoFile(
    content.map((it) => `${it.id}: ${it.tag}`).join('\n'),
    localTestReportPath
  );
}
...
function formatLocalTest(tests) {
  return tests.map((t) => {
    t.tag = sortedDistinct(
      t.tag.split(',').map((f) => getExceptOrDefault(f))
    ).join(',');
    return t;
  });
}
...
async function getAddedTestCases() {
  ...
  await exec(
    `git -c advice.detachedHead=false checkout $(git merge-base ${pr.head.sha} origin/${pr.base.ref})`
  );
...
async function getAffectedTestCases() {
  const idPattern = /\d+/g;
  const changePattern = /(^|\n)[+\\-].+/gm;
  let mergeBase = await spawn('git merge-base HEAD origin/main', {
    encoding: 'UTF-8',
  });
  ...
  const ids = [...diff.matchAll(methodRegexPattern)]
    ...
    .flatMap((group) => [...group[0].matchAll(testCaseAnnotationPattern)])
